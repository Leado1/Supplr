...
export async function generateInventoryPDF(data: ReportData): Promise<void> {
  try {
    ...
    if (data.summary) {
      ...
      const summaryData = [
        ...
        [
          ...
          data.summary.expired > 0 ? "#DC2626" : "#6B7280",
        ],
        [
          "Expiring Soon",
          data.summary.expiringSoon.toString(),
          data.summary.expiringSoon > 0 ? "#D97706" : "#6B7280",
        ],
        [
          "Low Stock Items",
          data.summary.lowStock.toString(),
          data.summary.lowStock > 0 ? "#EA580C" : "#6B7280",
        ],
      ];
          ...
          if (data.row.index >= 0) {
            ...
            if (data.column.index === 1 && data.row.index >= 0) {
              ...
              if (rowData && rowData[2]) {
                ...
                if (colorCode === "#DC2626")
                  data.cell.styles.textColor = dangerColor; // Red for expired
                else if (colorCode === "#D97706")
                  data.cell.styles.textColor = warningColor; // Amber for expiring
                else if (colorCode === "#EA580C")
                  data.cell.styles.textColor = warningColor; // Orange for low stock
                else if (colorCode === "#059669")
                  data.cell.styles.textColor = successColor; // Green for money/value
      ...
      didParseCell: function (data: any) {
        ...
        if (data.row.index >= 0 && data.row.index % 2 === 0) {
          if (data.column.index !== 7) {
            // Don't override status column colors
    ...
    if (data.items.length > 0) {
      ...
      doc.text(
        `Report contains ${data.items.length} inventory item${data.items.length === 1 ? "" : "s"}`,
        20,
        57
      );
    }
